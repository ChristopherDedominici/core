...
import { ether, MAX_SANE_SETTLED_GROWTH } from "lib/index.js";
import {
...
describe("Integration: OperatorGrid", () => {
  ...
  describe("Jail Status", () => {
    ...
    it("Jail blocks only minting and allows other operations", async () => {
      ...
      await expect(reportVaultDataWithProof(ctx, stakingVault, { totalValue: await dashboard.totalValue() })).to.not
        .revert(ethers);
