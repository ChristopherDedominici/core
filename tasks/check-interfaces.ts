...
export const checkInterfacesTask = task("check-interfaces")
    ...
    for (const contractFqn of artifactNames) {
      ...
      for (const { interfaceName, interfaceFqn } of interfaces) {
        ...
        try {
          ...
          function getFunctionSignaturesWithNames(abi: AbiItem[]): string[] {
            return abi
          ...
          function getFunctionSignaturesWithoutNames(abi: AbiItem[]): string[] {
            return abi
              .filter((item) => item.type === "function")
              .map((func) => {
          ...
          const interfaceSignaturesWithNames = getFunctionSignaturesWithNames(interfaceAbi as AbiItem[]);
          const contractSignaturesWithNames = getFunctionSignaturesWithNames(contractAbi as AbiItem[]);
          const interfaceSignaturesWithoutNames = getFunctionSignaturesWithoutNames(interfaceAbi as AbiItem[]);
          const contractSignaturesWithoutNames = getFunctionSignaturesWithoutNames(contractAbi as AbiItem[]);
